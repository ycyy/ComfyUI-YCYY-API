...
const NODE_CLASS = "YCYY_Grok_Image_API";
let apiMap = new Map();

async function loadApis() {
    try {
        const response = await api.fetchApi("/ycyy/grok/apis/all");
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
        apiMap = new Map((Array.isArray(data) ? data : []).map(item => [item["api-name"], item]));
    } catch (error) {
        console.error("[YCYY] Failed to load Grok API list:", error);
    }
}
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, appInstance) {
        ...
        nodeType.prototype.onConfigure = function () {
            ...
            if (apiWidget && apiMap.size && !apiMap.has(apiWidget.value)) {
                ...
                console.warn(`[YCYY] Grok API "${apiWidget.value}" no longer exists; using "${fallback}"`);
